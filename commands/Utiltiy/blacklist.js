...
module.exports = {
    ...
    callback: async (message,args) => {
        ...
        if(isblacklisted == null){
            ...
            await blmongo.save();
            message.channel.send({content:`<a:checkmark:870842284202164244> ${mentionmember} has been blacklisted with the ID of \`${id}\`.`,allowedMentions: {parse: []}})
...
const options = { returnNewDocument: true };
   await blacklistmongo.findOneAndUpdate(query, update, options).then(async updatedDocument => {
    ...
    if(updatedDocument) {
        ...
        logchannel.send({embeds: [logembed]})
        message.channel.send({content: `<a:checkmark:870842284202164244> ${mentionmember} has been unblacklisted with the ID of \`${blid}\`.`,allowedMentions: {parse: []}})
       
      
      console.log(`Successfully updated document: ${updatedDocument}.`)
    } else {
      message.reply(`Something went wrong! \`Cannot find document\``)
    }
